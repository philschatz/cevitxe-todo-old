...
import { getKeys } from '@philschatz/cevitxe/dist/src/keys'
...
import { wordPair } from '@philschatz/cevitxe-toolbar/dist/src/wordPair'
import { Status } from '@philschatz/cevitxe-toolbar/dist/src/Status'
...
export const Toolbar = ({
  ...
  // Hooks
  let [discoveryKey, setDiscoveryKey] = useQueryParam('id', StringParam)
  ...
  if (discoveryKey)
    getKeys('todo', discoveryKey)

  if (!discoveryKey && storeManager.knownDiscoveryKeys.length > 0)
    discoveryKey = storeManager.knownDiscoveryKeys[storeManager.knownDiscoveryKeys.length - 1]
  ...
  const url = (discoveryKey: string = '') =>
    `${window.location.protocol}//${window.location.host}/?id=${discoveryKey}`
